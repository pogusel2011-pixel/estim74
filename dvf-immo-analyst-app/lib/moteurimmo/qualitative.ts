...
export function buildQualitativeComparisons(
  ...
  return listings.map((listing) => {
    ...
    if (subject.surface !== listing.surface) {
      ...
      factors.push({ label: "Surface", impact: diff > 0 ? "positive" : "negative" as const, value: `${subject.surface} vs ${listing.surface} m²` });
    }
    if (subject.dpeLetter && listing.dpe && subject.dpeLetter !== listing.dpe) {
      ...
      factors.push({ label: "DPE", impact: better ? "positive" : "negative" as const, value: `${subject.dpeLetter} vs ${listing.dpe}` });
    }
    if (subject.hasPool && !listing.features?.includes("piscine")) {
      factors.push({ label: "Piscine", impact: "positive" as const, value: "Présente" });
    }
